...
class satDat:
    ...
    def read(self, invars, myqavar):
        ...
        self.qa_fordf = [
            "".join(
                [
                    str(self.qas_flat[i]),
                    str(self.qas_flat[i + 1]),
                    str(self.qas_flat[i + 2]),
                    str(self.qas_flat[i + 3]),
                    str(self.qas_flat[i + 4]),
                    str(self.qas_flat[i + 5]),
                    str(self.qas_flat[i + 6]),
                    str(self.qas_flat[i + 7]),
                    str(self.qas_flat[i + 8]),
                    str(self.qas_flat[i + 9]),
                ]
            )
            for i in range((self.qas_flat.shape[0]) - 9)
        ]
        ...
        self.dfind = [
            self.qa_fordf[0][q][0:2] == "51" for q in range(self.qa_fordf.shape[0])
        ]
    ...
    def mask_nans(self, qavar):
        ...
        self.vars[qavar][
            self.vars[qavar] == mymask
        ] = np.nan  # strings are an opportunity for errors
    ...
    def format_time(self, timevar):
        ...
        self.mytimes = [
            self.orig_date + timedelta(seconds=self.fvars[timevar][0][x])
            for x in range(len(self.fvars[timevar]))
        ]
    ...
    def make_df(self):
        ...
        self.df = pd.concat(
            self.frames, axis=1, ignore_index=True, verify_integrity=True
        )
