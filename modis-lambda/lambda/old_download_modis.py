...
def getMOD07L2Data(myind):
    ...
    # Set NaNs
    vars["Retrieved_Temperature_Profile"][
        vars["Retrieved_Temperature_Profile"] == -32768
    ] = np.nan
    vars["Retrieved_Moisture_Profile"][
        vars["Retrieved_Moisture_Profile"] == -32768
    ] = np.nan
    ...
    vars["Retrieved_Moisture_Profile"] = ret_temp_sf * (
        vars["Retrieved_Moisture_Profile"] - ret_temp_ao
    )
    ...
    mytimes = [
        orig_date + timedelta(seconds=fvars["Scan_Start_Time"][0][x])
        for x in range(len(fvars["Scan_Start_Time"]))
    ]

    fvars["Scan_Start_Time"] = pd.DataFrame(mytimes)

    frames = (
        fvars["Latitude"].reset_index(),
        fvars["Longitude"].reset_index(),
        fvars["Scan_Start_Time"].reset_index(),
        fvars["Cloud_Mask"].reset_index(),
        fvars["Surface_Pressure"].reset_index(),
        fvars["Surface_Elevation"].reset_index(),
        fvars["Processing_Flag"].reset_index(),
        fvars["Retrieved_Temperature_Profile"].reset_index(),
        fvars["Retrieved_Moisture_Profile"].reset_index(),
        fvars["Water_Vapor"].reset_index(),
        fvars["Water_Vapor_Low"].reset_index(),
        fvars["Water_Vapor_High"].reset_index(),
        fvars["Quality_Assurance"].reset_index(),
    )
...
test = pd.read_parquet(
    "s3://ncai-humidity/MODIS/MOD07_L2/MOD07_L2.A2000057.1810.061.2017202185019.parquet"
)
