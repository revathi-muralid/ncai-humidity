...
s3west_sesh = boto3.Session(region_name='us-west-2',aws_access_key_id=os.environ['NSAND_ACCESS'], aws_secret_access_key=os.environ['NSAND_SECRET'],aws_session_token=os.environ['NSAND_SESSION'])

s3east_sesh = boto3.Session(region_name='us-east-1',aws_access_key_id=os.environ['AWS_ACCESS_KEY_ID'], aws_secret_access_key=os.environ['AWS_SECRET_ACCESS_KEY'],aws_session_token=os.environ['AWS_SESSION_TOKEN'])
...
def getMOD07L2Data(myind):
    ...
    wr.s3.download(path=f"s3://prod-lads/MOD07_L2/{f1}", local_file=f"/tmp/{f1}", boto3_session=s3west_sesh)
    ...
    if d1.qa_fordf.shape[0]!=0:
        ...
        try:
            start = datetime.now()    
            wr.s3.to_parquet(d1.df, path="s3://ncai-humidity/MODIS/MOD07_L2/" + d1.outname + ".parquet", boto3_session = s3east_sesh)
